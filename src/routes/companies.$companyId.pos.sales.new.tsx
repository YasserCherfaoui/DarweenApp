...
export const NewSaleRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: '/companies/$companyId/pos/sales/new',
  component: NewSalePage,
})

function NewSalePage() {
  const { companyId } = NewSaleRoute.useParams()
  ...
  const handlePrintReceipt = async () => {
    ...
    try {
      const blob = await apiClient.pos.sales.getReceipt(Number(companyId), completedSale.id)
    ...
    createSale.mutate(
      ...
      },
      {
        onSuccess: (saleResponse) => {
          ...
          // Show success toast for sale creation
          toast({
          ...
          addPayment.mutate(
            ...
            },
            {
              ...
              onError: (error: any) => {
                ...
                // Still show print dialog even if payment fails
                setPrintTicketDialogOpen(true)
              },
            }
          )
  ...
  return (
    <RoleBasedLayout>
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button
          variant="ghost"
          onClick={() => navigate({ to: '/companies/$companyId/pos', params: { companyId } })}
        >
      ...
      {completedSale && (
        <ReceiptPreview
          ...
          onOpenChange={(open) => {
            ...
            if (!open) {
              navigate({ to: '/companies/$companyId/pos', params: { companyId } })
            }
          }}
          sale={completedSale}
