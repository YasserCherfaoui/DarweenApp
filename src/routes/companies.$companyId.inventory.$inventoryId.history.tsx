...
export const CompanyInventoryHistoryRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: '/companies/$companyId/inventory/$inventoryId/history',
  component: CompanyInventoryHistoryPage,
})

function CompanyInventoryHistoryPage() {
  const navigate = useNavigate()
  const { companyId, inventoryId } = CompanyInventoryHistoryRoute.useParams()
  const companyIdNum = Number(companyId)
  const inventoryIdNum = Number(inventoryId)

  const { data: inventoryList, isLoading } = useCompanyInventory(companyIdNum)
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate({ to: '/companies/$companyId/inventory', params: { companyId } })}
          >
