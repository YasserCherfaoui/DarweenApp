...
import { useCompanyInventory, useAdjustInventoryStock } from '@/hooks/queries/use-inventory'
...
export function VariantStockAdjustmentDialog({
  ...
  const { data: inventoryList } = useCompanyInventory(companyId)
  const [inventory, setInventory] = useState<Inventory | null>(null)

  // Find inventory for this variant
  useEffect(() => {
    if (inventoryList && variantId) {
      const inv = inventoryList.find((inv: Inventory) => inv.product_variant_id === variantId && !inv.franchise_id)
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Inventory not found for variant SKU: {variantSku || variantId}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>Adjust Stock</DialogTitle>
            <DialogDescription>
              Adjust the inventory stock for {variantSku || `variant #${variantId}`}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={!isValidAdjustment || adjustStock.isPending || !inventory}>
